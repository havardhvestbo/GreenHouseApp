...
### Data Simulation
 The two sensors are simulated with virtual data. To provide realistic data a max and min double are set for both the temperature and humidity sensor.
 Random data between those limits are provided to the server. To make the data consistant around the most realistic temperature and humidity, a normal 
 number is set. The data provided from the simulation will be close to the normal constribution with the help of a random delta. Generated by this formula (max - min) / 50.0.
